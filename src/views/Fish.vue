<template>
  <div v-if="getCollections" class="background">
    ...
    <div class="flex">
      <div class="card" v-for="(fish, i) in fishFilter" :key="i" :id="fish.name">
        <div class="container" @click="toggle_months = fish.id, hideMonths(fish)">
          <img :src="'/fish/'+fish.name + '.png'" />
          <h5>
            <span v-if="toggle_hemisphere">North -</span>
            <span v-else>South -</span>
            {{fish.name.replace(/_/g, " ")}} - {{fish.price}}
            <img
          ...
          <div
            :id="'arrow' + fish.id"
            alt="arrow"
            :class=" toggle_months != fish.id ? 'arrow-down' : 'arrow-up'"
          ></div>
          <div
            class="flex"
            :id="fish.id"
            :class="toggle_months == fish.id ? 'not-hidden' : 'hidden'"
            v-if="toggle_hemisphere"
          >
            <p class="padding" v-for="(north, i) in fish.northern_hemisphere" :key="i">{{north}}</p>
          </div>
          <div
            class="flex"
            :id="fish.id"
            ...
            v-else
          >
            <p class="padding" v-for="(south, i) in fish.southern_hemisphere" :key="i">{{south}}</p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...mapActions(["getData"]),
    hideMonths(fish) {
      let fishEl = document.getElementById(fish.id);
      if (fishEl.classList.contains("not-hidden")) {
        this.toggle_months = "";
      }
    },
    showMediaNavBar() {
  ...
  computed: {
    ...
    fishFilter() {
      ...
      if (this.filter_by_rarity.length == 0) {
        data_filter = this.getCollections.fish.filter(fish => {
          let fishName = fish.name.replace(/_/g, " ").toLowerCase();
          return fishName.includes(this.search.toLowerCase());
        });
      ...
      data_filter = this.getCollections.fish
        .filter(fish => {
          let fishName = fish.name.replace(/_/g, " ").toLowerCase();
          return fishName.includes(this.search.toLowerCase());
        })
        .filter(fish => {
          return this.filter_by_rarity.includes(fish.rarity);
        });
